    ...
    def schedule_generation(self,
        ...
        if validated:
            ...
            if acceptable_individual:
                ...
                result_working_time_slots = [ts.serialize() for ts in acceptable_individual.schedule.time_slots if isinstance(ts, WorkingTimeSlot)]
                result_working_time_slots.sort(key=lambda x: x['start_time'])
                print(acceptable_individual.schedule.time_slots)
                res = {
                    'result': result_working_time_slots
                }
